...
# TODO: Check, if i18n:attributes are missing, see './archive/setI18nAttrs.py' .
...
for root, dirs, files in os.walk("."):
    ...
    for file_name in files:
        ...
        if len(splitted_name) > 0:
            ...
            # It's a pagetemplate:
            if (suff in wanted_file_types) and (file_path.find('profile') != -1) and (file_path.find('upgrades') == -1) and (file_path.find('uninstall') == -1) and ( (file_path.find('type') != -1) or (file_name in (xml_own or xml_plone)) ):
                print file_path
                food = open(file_path)
...
# [u"\n    Tag's starting text\n    ", <childtag> childtext </childtag>', u'\n    ending text of tag\n    ']
                    ...
                    # TAG HAS TEXT:
                    #if tag_txt != '' and tag_txt != 'False' and tag_txt != 'True' and tag_txt != '-*- extra stuff goes here -*-' and not tag_txt.startswith('string:'):
                    if tag_txt != '' and tag.name != 'object' and tag_txt !='False' and tag_txt != 'True' and tag_txt != '-*- extra stuff goes here -*-' and not tag_txt.startswith('string:'):
...
# DEV:          result = open(file_path + '.tmp', 'w')
                result = open(file_name + '.tmp', 'w')
                result.write(str(soup))
                result.close()
# DEV:           shutil.move(file_path + '.tmp', file_path)
